...
const ContentAdminWrapper = ({ user, ...props }) => {
  const router = reactExports.useMemo(() => {
    const allRoutes = getCoordinatorRoutes(user, props);
    const contentAdminRoutes = allRoutes.filter((route) => {
      const path = route.path;
      return path === "/" || path === "/Home/Index" || path.includes("ManagePublicRegistrationForm") || path.includes("OpenAiCompetitionStructure") || path.includes("PublishToSocialMedia") || path.includes("Public/ViewEvent") || path === "*";
    });
    const routesWithErrorHandling = contentAdminRoutes.map((route) => ({
      ...route,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(reactExports.Suspense, { fallback: /* @__PURE__ */ jsxRuntimeExports.jsx(Loader, { from: "ContentAdminWrapper-Router" }), children: /* @__PURE__ */ jsxRuntimeExports.jsx(RouterProvider2, { router }) });
};
export {
  ContentAdminWrapper as default
};
