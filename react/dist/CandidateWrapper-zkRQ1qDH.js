...
const getCandidateRoutes = (user, context = {}) => {
  return [
    // Home and dashboard
...
const CandidateWrapper = ({ user, ...props }) => {
  const router = reactExports.useMemo(() => {
    const candidateRoutes = getCandidateRoutes(user, props);
    const routesWithErrorHandling = candidateRoutes.map((route) => ({
      ...route,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(reactExports.Suspense, { fallback: /* @__PURE__ */ jsxRuntimeExports.jsx(Loader, { from: "CandidateWrapper-Router" }), children: /* @__PURE__ */ jsxRuntimeExports.jsx(RouterProvider2, { router }) });
};
export {
  CandidateWrapper as default
};
