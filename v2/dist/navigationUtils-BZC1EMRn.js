import { r as reactExports, R as React, a as requireReactDom, g as getDefaultExportFromCjs } from "./main-B7w5eCOl.js";
var __typeError = (msg) => {
  throw TypeError(msg);
};
...
const getUrlPattern = () => {
  ...
  if (currentHref.includes("index.html")) {
    return "index.html";
  } else if (currentPath.startsWith("/v2")) {
    return "v2";
  } else {
    return "base";
  }
};
const generateRoutes = (basePath, element, options = {}) => {
  const routes = [];
  let normalizedPath;
  if (basePath === "") {
    normalizedPath = basePath;
  } else {
    normalizedPath = basePath.startsWith("/") ? basePath : `/${basePath}`;
  }
  if (basePath === "") {
    routes.push({
      path: "/",
    ...
    routes.push({
      path: `/v2${normalizedPath}`,
      element,
      ...options
    });
    routes.push({
      path: `/v2/index.html${normalizedPath}`,
      element,
      ...options
    });
  }
...
const smartNavigate = (navigate, targetPath) => {
  ...
  switch (urlPattern) {
    case "index.html":
      navigate(`/v2/index.html${fullPath}`);
      break;
    case "v2":
      navigate(`/v2${fullPath}`);
      break;
    case "base":
    default:
      navigate(fullPath);
...
const smartNavigateWithParams = (navigate, basePath, params = {}) => {
  let targetPath = basePath;
  Object.entries(params).forEach(([key, value]) => {
    targetPath = targetPath.replace(`:${key}`, encodeURIComponent(value));
  });
  smartNavigate(navigate, targetPath);
};
const getLink = (targetPath) => {
  const urlPattern = getUrlPattern();
  let fullPath = targetPath;
  if (!targetPath.startsWith("/")) {
    fullPath = `/${targetPath}`;
...
const getLinkWithParams = (basePath, params = {}) => {
  let targetPath = basePath;
  Object.entries(params).forEach(([key, value]) => {
    targetPath = targetPath.replace(`:${key}`, encodeURIComponent(value));
  });
  return getLink(targetPath);
};
const useSmartNavigate = (navigate) => {
  return (targetPath) => smartNavigate(navigate, targetPath);
};
const useSmartNavigateWithParams = (navigate) => {
  return (basePath, params = {}) => smartNavigateWithParams(navigate, basePath, params);
};
export {
  Navigate as N,
  Outlet as O,
  ...
  useNavigate as a,
  useSmartNavigate as b,
  createBrowserRouter as c,
  useLocation as d,
  getLink as e,
  useParams as f,
  generateRoutes as g,
  getLinkWithParams as h,
  useSmartNavigateWithParams as i,
  ReactDOM as j,
